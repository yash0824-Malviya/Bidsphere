/**
 * Creates / updates BidSphere RFP proposal response tracking on ERPNext:
 *   1. DocType "RFP Response" — supplier proposal submission
 *   2. Tracking fields on child table "RFP Supplier"
 *
 * Usage: node scripts/setup-rfp-response-doctype.mjs
 */
...
const RESPONSE_DOCTYPE = "RFP Response";
const SUPPLIER_CHILD = "RFP Supplier";
...
async function ensureSupplierChildTracking() {
  ...
  try {
    ...
    console.warn(
      `  ! "${SUPPLIER_CHILD}" missing — run scripts/setup-rfp-doctype.mjs first`,
    );
    return;
  }
...
async function main() {
  console.log("BidSphere RFP Response sync setup");
