...
interface Props {
  grn: PurchaseReceipt;
  /** When true, show compact spacing (Finance GRN detail). */
  compact?: boolean;
}

/**
 * Warehouse Verification summary (audit) used on Finance GRN detail and Voucher pages.
 * Document actions only — Verify Signature lives solely on the GRN Signed Document
 * "Digital Signature Information" card.
 */
...
function ActionBtn({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className="inline-flex items-center gap-1.5 rounded-lg border border-neutral-200 bg-white px-2.5 py-1.5 text-[11px] font-semibold text-neutral-700 shadow-sm hover:bg-primary-50 hover:text-primary-800 disabled:opacity-50"
    >
      {disabled ? (
