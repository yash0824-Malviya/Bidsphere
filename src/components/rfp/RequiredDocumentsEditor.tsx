...
import {
  RFP_ALLOWED_FILE_TYPES,
  RFP_DOCUMENT_TYPES,
  type RfpRequiredDocumentType,
} from "../../types/rfp";
import { generateId } from "../../utils/id";

export interface RfpDocumentDraft {
  id: string;
...
export function createDefaultRfpDocuments(): RfpDocumentDraft[] {
  ...
  return RFP_DOCUMENT_TYPES.map((doc_type) => ({
    id: generateId(),
    doc_type,
    required: true,
...
interface Props {
  docs: RfpDocumentDraft[];
  onChange: (next: RfpDocumentDraft[]) => void;
  readOnly?: boolean;
}
...
export default function RequiredDocumentsEditor({
  ...
  const update = (id: string, patch: Partial<RfpDocumentDraft>) => {
  ...
  return (
    <div className="space-y-3">
      <p className="text-[12px] text-[#64748B]">
        Configure documents suppliers must upload. Examples: Technical Proposal,
        Commercial Proposal, Company Profile, Project Plan, References, Brochure.
      </p>
